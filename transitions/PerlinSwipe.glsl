// author: akella
// license: other
// https://github.com/akella/webGLImageTransitions/blob/master/js/demo1.js

uniform float width; // = 0.5
uniform float transitionDuration; // = 1.
uniform float scaleX; // = 40.
uniform float scaleY; // = 40.
...
vec4 transition(vec2 uv)	{
  float time = progress * transitionDuration;
	...
	float border = 1.;
	vec2 newUV = uv;//(uv - vec2(0.5)) / ratio + vec2(0.5);
	vec4 color1 = getFromColor(newUV);
	vec4 color2 = getToColor(newUV);
	...
	float maskvalue0 = smoothstep(1.,1.,uv.x + progress);
